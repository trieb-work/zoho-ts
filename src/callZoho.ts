...
export abstract class MultiMethods {
        ...
        await retry(
            async () => {
                ...
                if (result?.data?.code !== 0)
                    throw new Error(
                        `Adding the address was not successfull! ${address}`,
                    );
    ...
    salesOrderEditpage = async () => {
        ...
        const customFieldReadyToFulfill = result.data.custom_fields.find(
            (x: { placeholder: string }) =>
                x.placeholder === "cf_ready_to_fulfill",
    ...
    getItem = async ({ product_id }: { product_id: string }) => {
        ...
        if (!result.data.item)
            throw new Error(
                `No Product data returned from Zoho! ${JSON.stringify(
                    result.data,
                )}`,
            );
    ...
    salesordersConfirm = async (salesorders: string[], retries = 3) => {
        ...
        for (const chunk of chunks) {
            const data = `salesorder_ids=${encodeURIComponent(
                chunk.join(","),
            )}`;
            await retry(
                async () => {
    ...
    getPublicInvoiceDownloadURL = async ({
        ...
        if (invoiceId) {
            ...
            if (!invoiceURL)
                throw new Error(
                    "Neither invoiceId, nor Invoice URL given! Failing..",
                );
    ...
    getDocumentFromSalesorderBase64String = async (
        ...
        const base64Buffer = Buffer.from(result.data, "binary").toString(
            "base64",
        );
...
export class ZohoClientInstance extends MultiMethods {
     ...
     * Returns the headers, that we send
     */
    authenticate = async () => {
